...
def _build_vision_tower(embed_dim: int,
    ...
    if vision_cfg.eva_model_name:
        ...
        visual = TimmModel(vision_cfg.timm_model_name,
...
def _build_text_tower(
    ...
    if text_cfg.hf_model_name:
        text = HFTextEncoder(text_cfg.hf_model_name,
                             output_dim=embed_dim,
...
def convert_to_custom_text_state_dict(state_dict: dict):
    if 'text_projection' in state_dict:
        ...
        for k, v in state_dict.items():
            if any(
                    k.startswith(p) for p in ('text_projection', 'positional_embedding', 'token_embedding',
...
def build_model_from_openai_state_dict(
    ...
    text_cfg = CLIPTextCfg(context_length=context_length,
