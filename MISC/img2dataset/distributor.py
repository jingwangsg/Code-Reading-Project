...
def retrier(runf, failed_shards, max_shard_retry):
    ...
    if len(failed_shards) != 0:
        print(f"Retried {max_shard_retry} times, but {len(failed_shards)} shards "
...
def singleprocess_distributor(processes_count, downloader, reader, _, max_shard_retry):
    """For debugging purposes, run the work in a single process"""

    def run(gen):
        failed_shards = []
        for status, row in tqdm(map(downloader, gen)):
            if status is False:
                failed_shards.append(row)
        return failed_shards
...
def _spark_session(processes_count: int):
    ...
    if spark is None:
        ...
        spark = (SparkSession.builder.config("spark.driver.memory",
                                             "16G").master("local[" + str(processes_count) +
                                                           "]").appName("spark-stats").getOrCreate())
    else:
        owned = False
