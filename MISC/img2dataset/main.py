...
from .distributor import (multiprocessing_distributor, pyspark_distributor, ray_distributor, singleprocess_distributor)
import fsspec
import sys
import signal
import os
...
def arguments_validator(params):
    ...
    if params["verify_hash"] is not None:
        ...
        if verify_hash_type != params["compute_hash"]:
            raise ValueError("verify_hash and compute_hash must be the same "
...
def download(
    ...
    elif distributor == "singleprocess":
        distributor_fn = singleprocess_distributor
    else:
