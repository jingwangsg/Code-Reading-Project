    ...
    for values in headers.get("X-Robots-Tag", []):
        try:
            ...
            if (ua_token is None or ua_token == user_agent_token) and any(
                ...
                # 只要满足以下两个条件就是disallowed
                # 1. 没有ua_token 或 ua_token和user_agent_token相同
                # 2. directives中有一个在disallowed_header_directives中
                return True
...
def download_image(row, timeout, user_agent_token, disallowed_header_directives):
    ...
    try:
        if False:  # compare method
            ...
            # ======================== WGET ========================
            # from kn_util.utils import CommandDownloader
            # import tempfile
            # with tempfile.NamedTemporaryFile() as f:
            #     CommandDownloader.download_wget(url=url,
            #                                     out=f.name,
            #                                     timeout=timeout,
        ...
        with httpx.Client(verify=False, timeout=timeout, follow_redirects=True) as client:
            r = client.get(url, headers={"User-Agent": user_agent_string})
            if disallowed_header_directives and is_disallowed(
                    r.headers,
                    user_agent_token,
                    disallowed_header_directives,
...
def compute_key(key, shard_id, oom_sample_per_shard, oom_shard_count):
    # 这里是为了key的长度一致，方便后续的排序，这里的oom是order of magnitude的缩写
    # 如shard_id=4, key=123, oom_sample_per_shard=5, oom_shard_count=4
    # 则str_key = "000040123", 前5位是shard_id, 后4位是key，使用0补齐
...
class Downloader:
    ...
    def __init__(
        ...
        self.disallowed_header_directives = (None if disallowed_header_directives is None else
    ...
    def download_shard(
        ...
        # ======================== Construct schema & shard_to_dl ========================
        fs, shard_path = fsspec.core.url_to_fs(shard_file)
        with fs.open(shard_path, "rb") as f:
            df = pa.ipc.open_file(f).read_all()

        schema = df.schema
        schema = (schema.append(pa.field("key", pa.string())).append(pa.field("status", pa.string())).append(
            pa.field("error_message",
                     pa.string())).append(pa.field("width", pa.int32())).append(pa.field("height", pa.int32())).append(
                         pa.field("original_width", pa.int32())).append(pa.field("original_height", pa.int32())))
        ...
        hash_indice = (self.column_list.index(self.verify_hash_type)
                       if self.verify_hash_type in self.column_list else None)
        bbox_indice = self.column_list.index(self.blurring_bbox_col) if self.blurring_bbox_col is not None else None
        key_url_list = [(key, x[url_indice]) for key, x in shard_to_dl]

        # Construct Sample Writer
        ...
        # ipdb.set_trace()
        # item = next(loader)
        # download_image_with_retry(item,
        ...
        with ThreadPool(self.thread_count) as thread_pool:
            # 这里的thread_pool.imap_unordered是一个generator，处理好一个就返回一个，没处理好就阻塞
            # 由于loader已经被信号量semaphore限制了，所以thread_pool.imap_unordered也会被限制
