...
class AdaMixerDecoder(CascadeRoIHead):
    ...
    def _bbox_forward(self, stage, img_feat, query_xyzr, query_content, img_metas):
        ...
        cls_score, delta_xyzr, query_content = bbox_head(
            img_feat, query_xyzr, query_content, featmap_strides=self.featmap_strides
        )
        ...
        if DEBUG:
            with torch.no_grad():
                torch.save(bbox_results, "demo/bbox_results_{}.pth".format(AdaMixerDecoder._DEBUG))
    ...
    def simple_test(self, x, query_xyzr, query_content, img_metas, imgs_whwh, rescale=False):
    ...
    def forward_dummy(self, x, query_xyzr, query_content, img_metas):
