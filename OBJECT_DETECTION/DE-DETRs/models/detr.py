    ...
    def __init__(self,
        ...
        if num_feature_levels > 1:
            ...
            self.input_proj = nn.ModuleList(
                [nn.Sequential(nn.Conv2d(backbone.num_channels[0], hidden_dim, kernel_size=1),)])
        ...
        if box_refine:
            ...
            # shared weight between all self.class_embed and self.bbox_embed
        ...
        """ <END> initialize class_embed, bbox_embed, ref_point_head"""
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, repeat_label=None, repeat_ratio=None):
        ...
        if self.training:
            if self.repeat_label is not None or self.repeat_ratio is not None:
                ...
                for batch_idx in range(len(targets)):
                    ...
                    targets[batch_idx]['repeat'] = torch.zeros(num_inst * repeat_time,
                                                               dtype=torch.int64,
...
def build(args):
    ...
    postprocessors = {
        'bbox':
            PostProcess(num_queries=args.num_queries,
                        nms=args.nms,
