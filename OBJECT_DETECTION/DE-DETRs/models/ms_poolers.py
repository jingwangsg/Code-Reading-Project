...
class MSROIPooler(nn.Module):
    ...
    def __init__(
        ...
        if pooler_type == "ROIAlign":
            self.level_poolers = nn.ModuleList(
                ROIAlign((size, size), spatial_scale=scale, sampling_ratio=sampling_ratio, aligned=False)
                for size, scale in zip(output_size, scales))
        elif pooler_type == "ROIAlignV2":
            self.level_poolers = nn.ModuleList(
                ROIAlign((size, size), spatial_scale=scale, sampling_ratio=sampling_ratio, aligned=True)
                for size, scale in zip(output_size, scales))
        elif pooler_type == "ROIPool":
            self.level_poolers = nn.ModuleList(
                RoIPool((size, size), spatial_scale=scale) for size, scale in zip(output_size, scales))
        ...
        assert (len(scales) == self.max_level - self.min_level +
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        assert len(box_lists) == x[0].size(
            0), "unequal value, x[0] batch dim 0 is {}, but box_list has length {}".format(
