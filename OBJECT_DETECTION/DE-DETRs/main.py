...
def get_args_parser():
    ...
    parser.add_argument('--wandb',
                        action='store_true',
                        help="turn on wandb for logging")
    # label augmentation
    parser.add_argument('--repeat_label',
                        type=int,
                        default=None,
                        help="repeat positive labels for n times")
    parser.add_argument(
        '--repeat_ratio',
        type=float,
        default=None,
        help="resample positive labels to make pos:all=ratio, e.g. 0.25")
    parser.add_argument('--two_stage_match',
                        action='store_true',
                        help="two stage matching for the repeated label")
    ...
    parser.add_argument('--nms_remove',
                        type=float,
                        default=0.01,
                        help="score multiplier for removed preds")
    ...
    parser.add_argument('--pool_res',
                        ...
                        default=4,
                        help="roi resolution)")
    parser.add_argument('--init_ref_dim',
                        type=int,
                        default=2,
                        help="dimension of init reference",
                        choices=[2, 4])
    parser.add_argument('--no_box_refine',
                        action='store_false',
                        dest='box_refine',
    ...
    parser.add_argument(
        ...
        dest='ms_roi',
        help=
        "update memory and pos by roi align on single-scale feature (32x down-sampled)"
    )
    ...
    parser.add_argument('--sample_rate',
                        default=None,
                        type=float,
                        help="sample rate for downsampled dataset")
    parser.add_argument(
        '--sample_repeat',
        action='store_true',
        help=
        "repeat the dataset 1/sample_rate times, to maintain the computational cost"
    )
...
def get_dataset_name(args):
    ...
    dataset_name = args.dataset_file if args.sample_rate is None else args.dataset_file + str(
        args.sample_rate)
...
def main(args):
    ...
    run_name = '_'.join([
        dataset_name,
        args.model,
        'bs{}x{}'.format(args.world_size, args.batch_size),
