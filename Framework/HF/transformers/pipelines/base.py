...
def infer_framework_from_model(
    ...
    **model_kwargs,
    ...
    if isinstance(model, str):
        ...
        config = model.config
    return infer_framework_load_model(model,
                                      config,
...
def get_default_model_and_revision(targeted_task: Dict, framework: Optional[str],
                                   task_options: Optional[Any]) -> Union[str, Tuple[str, str]]:
    """
...
class PipedPipelineDataFormat(PipelineDataFormat):
    ...
    def save_binary(self, data: Union[dict, List[dict]]) -> str:
        if self.output_path is None:
            raise KeyError("When using piped input on pipeline outputting large object requires an output file path. "
...
class Pipeline(_ScikitCompat):
    ...
    def get_iterator(self, inputs, num_workers: int, batch_size: int, preprocess_params, forward_params,
                     postprocess_params):
        if isinstance(inputs, collections.abc.Sized):
            dataset = PipelineDataset(inputs, self.preprocess, preprocess_params)
        else:
            if num_workers > 1:
                logger.warning("For iterable dataset using num_workers>1 is likely to result"
    ...
    def __call__(self, inputs, *args, num_workers=None, batch_size=None, **kwargs):
        ...
        if is_list:
            if can_use_iterator:
                # 返回一个迭代器
                final_iterator = self.get_iterator(inputs, num_workers, batch_size, preprocess_params, forward_params,
                                                   postprocess_params)
                # 转化为列表
            ...
            # not list, but can use iterator
            return self.get_iterator(inputs, num_workers, batch_size, preprocess_params, forward_params,
                                     postprocess_params)
        elif is_iterable:
            # 返回一个迭代器
            ...
            return next(
                iter(
                    self.get_iterator([inputs], num_workers, batch_size, preprocess_params, forward_params,
                                      postprocess_params)))
        else:
